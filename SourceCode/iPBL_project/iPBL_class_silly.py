...
class FaceRecognition:
    def __init__(self, playerNumber):
        ...
        self.KEY = [[1,10], [1, 10], [1, 10], [1, 10], [1,10], [130, 243], [359, 362]]

    def caluclate_distance(self, point1, point2):
    ...
    def takephoto(self, mode):
        ...
        while cap.isOpened() :
            ...
            cv2.line(img, pt1=(int(wt/2), 0), pt2=(int(wt/2), int(ht)), color=(255, 255, 255)) 
    ...
    def noramlization(self, face):
        distance = [[0 for _ in range(len(self.FACE_PARTS[i]))] for i in range(len(self.FACE_PARTS))]
        for i in range(len(self.FACE_PARTS)):
            normal = self.caluclate_distance(face[self.KEY[i][0]], face[self.KEY[i][1]])
            for index, j in enumerate(self.FACE_PARTS[i]):
                distance[i][index] = self.caluclate_distance(face[self.KEY[i][0]], face[j])
                distance[i][index] /= normal
    ...
    def caluclate_diff(self, distance1, distance2):
        ...
        for i in range(len(self.FACE_PARTS)):
            for j in range(len(self.FACE_PARTS[i])):
                result[i][j] =  distance1[i][j] - distance2[i][j]
        return result

    def calculate_score(self, diff1, diff2):
        diff_sum = [0 for _ in range(len(self.FACE_PARTS))]
        for i in range(len(self.FACE_PARTS)):
            for j in range(len(self.FACE_PARTS[i])):
    ...
    def main(self):
        for i in range(self.playerNumber):
            self.straight_faces.append(self.takephoto('noraml'))
            self.silly_faces.append(self.takephoto('silly'))
            straight_detect = self.Face.detect(self.straight_faces[i])
            silly_detect = self.Face.detect(self.silly_faces[i])
            straight_result = self.noramlization(straight_detect.face_landmarks[0])
            silly_result = self.noramlization(silly_detect.face_landmarks[0])

            self.diff.append(self.caluclate_diff(straight_result, silly_result))

        score = self.calculate_score(self.diff[0], self.diff[1])
        print(score)
